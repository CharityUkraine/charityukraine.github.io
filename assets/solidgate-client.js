...
class SolidgateClient {
    ...
    validateForm({ customer_email = "", amount = 0 })
    {
        ...
        let emailErrorElement = document.querySelector(`${this.parentSelector} .${DONATE_EMAIL}-error`);
        if (emailErrorElement) {
            emailErrorElement.style.display = invalidEmail ? 'block' : 'none';
        }

        let amountErrorElement = document.querySelector(`${this.parentSelector} .${DONATE_AMOUNT}-error`);
        if (amountErrorElement) {
            amountErrorElement.style.display = invalidAmount ? 'block' : 'none';
        }
    ...
    async activateDonateForm() {
        ...
        const checkedCurrencyBtnSelector = `${this.parentSelector} [name="${DONATE_CURRENCY}"]:checked`;

        let amount = document.querySelector(`${this.parentSelector} #${DONATE_AMOUNT}`).value,
            customer_email = document.querySelector(`${this.parentSelector} #${DONATE_EMAIL}`).value,
            currency = document.querySelector(checkedCurrencyBtnSelector).value,
            language = document.querySelector('html').lang,
            product_id = this.getProductId(),
            project_id = document.querySelector('#solidgate-data-projects_id')?.textContent,
